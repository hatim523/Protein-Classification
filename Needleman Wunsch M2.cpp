...
public:
	...
	void NeedleManWunsch()	//Penalty on Mismatch: -1 //Reward on Match: +1
	{
	...
	for (int l = 0; l < 10; l++)
	{
		...
		for (int i = 0; i < totalClasses; i++)
		{
			...
			for (int j = 0; j < SubClasses; j++)
			{
				...
				scores[k++] = s1.ClassInput1(classstr);
			}
		...
		//int topscores[3];
		int **topscores = Find3BestScores(scores, 80);

		cout << "For Class " << l << endl;
		bool onetime = false;
		for (int i = 0; i < 3; i++)
		{
			cout << "Score: " << topscores[i][0] << " Class: " << topscores[i][1] / 8 << endl;
			if (l == topscores[i][1] / 8 && !onetime)
			{
				onetime = true;
				correctans++;
			}
